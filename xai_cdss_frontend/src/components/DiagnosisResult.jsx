  ...
  return (
    <div className="bg-card rounded-2xl shadow-card border border-border overflow-hidden animate-slide-up">
      <div className="p-6 md:p-8">
        ...
        <div className={`p-6 rounded-xl border-2 ${isPneumonia
          ? 'bg-danger-light border-danger/30'
          : 'bg-success-light border-success/30'
          }`}>
          <div className="flex items-start gap-4">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center flex-shrink-0 ${isPneumonia ? 'bg-danger/10' : 'bg-success/10'
              }`}>
              {isPneumonia ? (
        ...
        <div className="mt-6 p-4 rounded-xl bg-muted/50">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`text-lg font-bold ${confidence >= 0.8 ? 'text-success' : confidence >= 0.6 ? 'text-warning' : 'text-danger'
              }`}>
          ...
          <div className="h-3 bg-muted rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-1000 ease-out ${confidence >= 0.8 ? 'bg-success' : confidence >= 0.6 ? 'bg-warning' : 'bg-danger'
                }`}
